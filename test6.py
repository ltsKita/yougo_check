...
def analyze_and_replace(text, log_file):
    ...
    for token in tokens:
        ...
        # 解析結果をログファイルに書き出し
        log_file.write(f"表層形: {surface}, 読み仮名: {pronunciation}\n")
...
def split_and_highlight_text_element(text_element, keywords, log_file):
    """
        ...
        for paragraph in root.findall('.//w:p', namespaces):
            ...
            if any(keyword in full_text for keyword in ["とき", "時", "他", "外"]):
                ...
                for text_elem in paragraph.findall('.//w:t', namespaces):
                    if text_elem.text:
                        split_and_highlight_text_element(text_elem, ["ほか"], log_file)  # 「ほか」に変換後ハイライトを適用
