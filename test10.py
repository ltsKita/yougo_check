...
def create_highlighted_run(original_rpr, text, color):
    ...
    if original_rpr is not None:
        new_rpr.extend(original_rpr)  # 元の<w:rPr>を引き継ぐ
...
def syntactic_analysis_and_highlight(doc, original_rpr, syntax_log_file, combined_text, combined_pronunciation):
    ...
    for token in doc:
        ...
        if surface in ["時", "とき"] and pronunciation == "トキ":
            ...
            # 「ジ」と読まれるものもログに出力
            syntax_log_file.write(f"無視: {surface}, 読み仮名: {pronunciation}, dep: {token.dep_}\n")
            modified_text.append(surface)
...
def split_and_highlight_text_element(text_element, log_file, syntax_log_file):
    ...
    形態素解析で「ほか」を意味する部分を「ほか」に変換し、構文解析で「とき」と「時」を適切に変換してハイライトを適用
    """
    ...
    for original_text, modified_text, color in highlighted_runs:
        # ハイライトされない部分を追加
        if current_position < syntactically_modified_text.find(modified_text):
            unhighlighted_text = syntactically_modified_text[current_position:syntactically_modified_text.find(modified_text)]
            plain_run = ET.Element('{http://schemas.openxmlformats.org/wordprocessingml/2006/main}r')
            plain_rpr = ET.SubElement(plain_run, '{http://schemas.openxmlformats.org/wordprocessingml/2006/main}rPr')
            if original_rpr is not None:
                plain_rpr.extend(original_rpr)  # 元の<w:rPr>を引き継ぐ
            plain_text_element = ET.SubElement(plain_run, '{http://schemas.openxmlformats.org/wordprocessingml/2006/main}t')
            plain_text_element.text = unhighlighted_text
            new_elements.append(plain_run)
            current_position = syntactically_modified_text.find(modified_text)

        # ハイライトされた部分を追加
        new_run = create_highlighted_run(original_rpr, modified_text, color)
        new_elements.append(new_run)
        current_position += len(modified_text)
    ...
    if current_position < len(syntactically_modified_text):
        ...
        if original_rpr is not None:
            plain_rpr.extend(original_rpr)  # 元の<w:rPr>を引き継ぐ
        plain_text_element = ET.SubElement(plain_run, '{http://schemas.openxmlformats.org/wordprocessingml/2006/main}t')
        plain_text_element.text = remaining_text
        new_elements.append(plain_run)
